...
function onNewPage(e, data){
	...
	$(document).trigger('resize').trigger('pageRendered', data);
}
...
function addNavProps(){
	...
	nav.update = function(n){
		if(arrowNav && nav.numPages>1){
			...
			if(n>=nav.numPages || (view.length && (view[0] === nav.numPages || view[1] === nav.numPages)) || nav.current >= nav.numPages) $('nav #nextBtn').hide();
		}
	};
...
function scaleFlipLayout(getOnly,sf) {
	...
	nav.reposition();
	if(scaleFactor >= 1.5) $('.activePage .mejs-video video').css('transform','translateZ(0)').redraw();
...
function getScaleFactor(targW, targH){
	...
	switch(scaleTo) {
		...
		case 'width': $(document.body).attr('data-scaled-to','w'); return Math.min(maxWF,$(window).innerWidth()/targW);
...
function onMediaLoadSuccess (me, domObj) {
	...
	if($domObj.attr('data-overlap')=='1') domObj.player.options.pauseOtherPlayers = false;
}
